...
- [ ] **UX pays : signalement créé hors du pays actif = invisible dans SA propre liste** (constaté 2026-07-28 en test prod : pays hérité des prefs ≠ pays GPS du signalement → le flux serveur scoppé l'exclut, l'utilisateur croit à un bug). Pistes : bannière « signalement créé dans un autre pays », ou aligner automatiquement le pays actif sur celui du signalement qu'on vient de créer, ou inclure « mes signalements » hors périmètre. Cas réel : diaspora qui a choisi le Cameroun manuellement et signale depuis son pays de résidence.
