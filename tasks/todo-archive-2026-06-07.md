# NJUKA — Archives todo (juin → début août 2026)

> Sections historiques déplacées depuis `todo.md` le 2026-08-18 (ménage).
> Tout est TERMINÉ ou obsolète ici — la vérité courante vit dans `todo.md`.
...
**2026-07-28 — 🚀 VERSION 59 SOUMISE SUR LES DEUX STORES** : App Store (review Apple en cours) + Play Store (rollout production). Pointeur `master` avancé sur `v1.2.0+59`. Surveiller : verdicts de review, Crashlytics, Analytics (funnel anonyme `anonymous_started → upgrade_completed`, incitations `report_denied`/`outage_prompt_dismissed`, `positionSource`), coûts Firebase njuka-prod. Restants post-lancement : App Check enforcement (après adoption), langue FR des emails Auth njuka-prod, AAB staging v60 pour les testeurs (Google masqué + targetSdk 36), tuile support (vérifier boîte `support@bogal.ca`).

**2026-07-28 — BUILD PROD iOS `1.2.0 (59)` UPLOADÉ sur ASC** ✅ (config njuka-prod via `use_env.sh prod`, archive signée, upload CLI — pièges documentés dans lessons ; options d'export versionnées : `ios/ExportOptions-appstore.plist`). Test iPhone réel fait le jour même (écrivait bien dans njuka-prod, base repurgée ensuite). Reste : associer le build 59 à la version 1.2.0 dans ASC → **soumettre à la review**.
...
**RESTE AVANT PUBLICATION PROD** : ~~① smoke-test Facebook Android~~ ✅ ; ~~② **jour J OAuth Google**~~ ✅ **FAIT 2026-07-28** (clients supprimés de lightcutoff-dev par l'utilisateur ; création côté njuka-prod déclenchée en recyclant les 3 SHA-1 via `firebase apps:android:sha:delete/create` — la suppression seule ne suffit PAS ; `google-services.prod.json` régénéré avec **3 clients OAuth type 1** + web, `firebase_options_prod.dart` a gagné l'`iosClientId`. ⚠️ **Staging a perdu Google Sign-In** comme prévu — prévenir les testeurs qui l'utilisaient) ; ③ build `.aab` prod (v58+) + rollout progressif Play 10 % ; ④ côté iOS : refaire un build prod (`use_env.sh prod`) et l'uploader pour la release App Store ; ⑤ App Check enforcement ; ⑥ retirer « RD Congo » des descriptions de fiches.
