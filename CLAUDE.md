...
- **`dev`** — the trunk, always green (CI), always releasable. Day-to-day work commits here. This is the default working branch.
- **`master`** — a POINTER to what is live in production, never committed to directly. Move it when a prod release ships: `git branch -f master v1.2.0+N && git push -f origin master`. `git log master..dev` = what dev/staging has that prod doesn't.
- **Tags `v1.2.0+N`** — every shipped build (staging closed-test or prod).
- **`feat/…`** — short-lived only (risky/multi-day work), merged into `dev` fast, deleted after. A stale feature branch is archived as a tag (`archive/<name>`) before deletion.
- **`hotfix/…`** — created LAZILY from `master` (the prod tag) only when prod needs a fix while dev has moved on; cherry-pick back to `dev`, delete after.
...
- **`RegionProvider`** — active country: dev override → user choice (**dev/staging only** since 2026-07-28; ignored in prod even if persisted) → auto-detect (GPS, then **IP fallback** via `api.country.is` — `lib/services/ip_country_service.dart`, one attempt per session, GPS always wins because VPNs skew IP) → profile → locale → CM. Also `activeUtility(ServiceType)` + persistent `serviceFilter` + 2 dev overrides (one per service) with symmetric auto-coupling. When a country is manually selected (dev/staging QA), report creation attaches the report to the SELECTED country (`createFromDraft(countryOverrideIso:)`) and the form shows an info banner if it differs from the detected one — so QA reports stay visible in the list being browsed.
...
## Report lifecycle (2026-08-09)

`reportLifecycle` CF (cron 30 min, **staging only until the v67 app ships** — older apps would render the wrong buttons): single "still out?" ping 4h after report time within the 7h-21h local window (once per person per report, `stillOutPingedUids`), silent expiration after 48h of inactivity (`archivedAt`+`autoExpiredAt` — NEVER counted as resolved nor in duration stats; any activity resets the clock). Ping notification is data-only with `kind: still_out_ping` → the app renders [Toujours coupé] [C'est revenu ✓] (notification_actions): "revenu" = background restoration vote; "toujours coupé" = `markStillOut` callable (Admin-side `updatedAt` touch — clients can't legally write it; no counters touched). Signaler loop: author gets "Tu n'es pas seul" at the 1st and 5th confirmation (`shouldNotifyAuthor`), the resolution notification carries an impact line (`impactLine`), and the report form shows the return promise before the gesture.

## App Links / Universal Links (2026-08-09)

`njuka.app/s/{id}` opens the app when installed. Verification files live in the WEBSITE repo (`../lightCutOff/src/well-known/` — assetlinks.json with THREE SHA-256s: Play signing key, upload key, debug key; AASA served as application/json via a hosting header). Android: autoVerify intent filter in the manifest (verified on device). iOS: `associated-domains` entitlement + capability on the App ID + regenerated "NJUKA AppStore" profile. In-app routing: `DeepLinkService` (app_links) feeds `NotificationService.pendingReportId` — same channel as notification taps.
...
- **Map draws outages as impact zones** (2026-08-07): each ongoing report renders as a translucent service-colored disc (`CircleLayer`, `useRadiusInMeter`) whose radius is `report.impactRadiusM` — an anonymous aggregate written ONLY by `onConfirmationCreated` (max epicenter↔confirmer distance, server-clamped [150, 2000] m, never shrinks; `nextImpactRadius` in logic.ts). Fill opacity encodes freshness of `updatedAt ?? reportedAt` (`lib/utils/impact_zone.dart`, pure + tested). Pins remain the tap targets; clustering was removed (zones carry density semantics); resolved reports are already excluded by the provider's default status filter (toggleable in the app-bar filter sheet — do NOT add a dedicated map toggle, that was a duplicate once removed). Individual confirmation positions stay rule-locked — the client only ever sees the radius.
